...
public class TestRunnerContextTests
{
    ...
    {
        var json = JsonConvert.DeserializeObject<Model.Test>(File.ReadAllText("Resources/Tests/pets-post.json"));
    ...
    {
        var json = JsonConvert.DeserializeObject<Model.Test>(File.ReadAllText("Resources/Tests/pets-post-generic.json"));
        var body = this.context.Eval(json.Request.Body);
        body.Should().NotBeNull();
        var dynBody = body as dynamic;
